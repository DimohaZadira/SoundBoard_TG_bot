...
async def add_sound_handler(message: types.message, state: FSMContext):
    ...
    if name == '':
        await message.answer('Please tell me how to name a sound you want to attach! Tell me /add_sound <sound_name> ❌🤔') 
    else:
...
async def get_audio(message: types.Document, state: FSMContext):
    try:
        ...
        await message.answer(f"Good job! Your sound has been successfully added! Try sending it to somebody using @play command! ✅😎")
        await state.finish()
    except:
